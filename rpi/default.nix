  ...
  config = {
    systemd.services = {
      "raspberry-pi-firmware-migrate" = {
        ...
        wantedBy =
          if cfg.firmware-migration-service.enable
        ...
        in {
          Type = "oneshot";
          MountImages = "/dev/disk/by-label/${cfg.firmware-partition-label}:${firmware-path}";
      ...
      overlays = let
        rpi-overlays =
          ...
          ++ (
            ...
            then [libcamera-overlay]
            else []
          );
        rpi-overlay = lib.composeManyExtensions rpi-overlays;
        pin-prev-overlay = overlay: pinned-prev: final: prev: let
    ...
    boot = {
      ...
      kernelPackages =
        if cfg.kernel-build-system == null
        then pkgs.linuxPackagesFor pkgs.rpi-kernels."${version}"."${board}"
        else pkgs.linuxPackagesFor (pkgs.rpi-kernels-cross cfg.kernel-build-system)."${version}"."${board}";
      loader = {
